...
function MobileTabs() {
  ...
  return (
    <>
      <div className="lg:hidden bg-white shadow-lg p-3 fixed bottom-0 z-10 w-full flex items-center  justify-evenly text-gray-500 border">
        {allTypes.map((items) => (
          <Link to={`/${items.id}`} key={items.id}>
            <div
              className={
                type === items.id
                  ? "flex flex-col relative text-xl text-zomato-400 align-middle"
            ...
            >
              <div
                className={
                  type === items.id
                    ? "flex flex-col w-full justify-center items-center  border-t-2 border-zomato-400 p-2"
                    : "flex flex-col justify-center items-center border-t-2 border-white"
                }
              >
                {items.icon}
...
function LargeTabs() {
    ...
    return (
    <>
        <div className="hidden lg:flex gap-14 container mx-auto px-20 my-8">
            {allTypes.map((items)=>(
                <Link to={`/${items.id}`} key={items.id}>
                    <div className={classnames("flex items-center gap-3 pb-2 transition duration-700 ease-in-out",{"border-b-2 border-zomato-400":type===items.id})}>
                        <div className={classnames("w-16 h-16 bg-gray-100 p-4 rounded-full",{[`bg-${items.activeColor}-100`]:type===items.id})}>
                            <img src={type===items.id ? items.imageActive : items.imageDefault} alt="options" className="w-full h-full"/>
                        </div>
